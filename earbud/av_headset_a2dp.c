...
/*! \brief Start audio, sending a delay report if supported. */
static void appA2dpStartAudio(avInstanceTaskData *theInst)
{
    ...
    if (codec_settings)
    {
        ...
        appKymeraA2dpStart(&theInst->av_task, codec_settings, appAvVolumeGet(),
            is_sink_non_tws_codec ? MESSAGES_FROM_SYNC_IND_TO_A2DP_START_REQ : 0);
        if (is_sink_non_tws_codec && codec_settings->codecData.latency_reporting)
        {
            /* Send delay report to audio source if it supports it.  Delay report
               units are 1/10ms, so multiply latency by 10 */
            A2dpMediaAvSyncDelayRequest(theInst->a2dp.device_id, theInst->a2dp.current_seid,
                                        TWS_STANDARD_LATENCY_MS * 10);
        }
        free(codec_settings);
    }
    else
...
{
    appKymeraA2dpStop(NULL, theInst->a2dp.current_seid, StreamSourceFromSink(theInst->a2dp.media_sink));
}
...
static void appA2dpEnterConnectingMediaLocal(avInstanceTaskData *theInst)
{
    ...
    if (theInst->a2dp.current_seid != AV_SEID_INVALID)
        ...
        A2dpMediaOpenRequest(theInst->a2dp.device_id, 0, NULL);
}
...
/*! \brief Find the start of the service category in CODEC capabilities
    ...
    Searches the CODEC capabilities for the required service category.
    caps set to category header or NULL if not found/malformed.
    size_caps set to total size of category or 0 if not found/malformed.

    *caps_ptr must point a service cateogry definition (followed by the serivce
    category length)
*/
                                                      ...
                                                      const AV_INTERNAL_A2DP_INST_SYNC_RES_T *res)
{
    ...
    if ((res->sync_id + 1) != theInst->a2dp.sync_counter)
    {
...
/* \brief Handle the KYMERA_A2DP_START_CFM message.

    This message is typically expected in the state
    A2DP_STATE_CONNECTED_MEDIA_STARTING_REMOTE_SYNC, but could be received
    in virtually any other state if whilst waiting for the confirm other
    A2DP events occurred (e.g. disconnect, suspend etc). So for now
    just clear the lock and ignore the current state.
*/
static void appA2dpHandleKymeraA2dpStartConfirm(avInstanceTaskData *theInst)
{
    DEBUG_LOGF("appA2dpHandleKymeraA2dpStartConfirm(%p)", theInst);
    appA2dpClearKymeraLockBit(theInst);
}
...
void appA2dpInstanceHandleMessage(avInstanceTaskData *theInst, MessageId id, Message message)
{
    ...
    /* Handle kymera messages */
    switch(id)
    {
        case KYMERA_A2DP_START_CFM:
            appA2dpHandleKymeraA2dpStartConfirm(theInst);
        return;
    }
