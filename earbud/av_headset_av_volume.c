...
static void appAvApplyVolume(uint8 volume)
{
...
void appAvVolumeHandleAvrcpConnect(avInstanceTaskData *theInst)
{
    ...
    {
        bdaddr hs_bd_addr;
        avInstanceTaskData *theOtherInst = NULL;
        PanicFalse(appDeviceGetHandsetBdAddr(&hs_bd_addr));
        PanicFalse(appDeviceGetVolume(&hs_bd_addr, &volume));

        if (appDeviceIsPeer(&theInst->bd_addr))
        {
            /* Assign handset inst so the peer inst volume is set */
            theOtherInst = appAvInstanceFindFromBdAddr(&hs_bd_addr);
        }
        DEBUG_LOGF("appAvVolumeHandleAvrcpConnect(%p), theOtherInst=%p, volume=%u",
                    theInst, theOtherInst, volume);
        appAvVolumeSet(volume, theOtherInst);

    }
    else
    {
        volume = theInst->avrcp.volume;
        DEBUG_LOGF("appAvVolumeHandleAvrcpConnect(%p), no handset, volume=%u", theInst, volume);
        appAvVolumeSet(volume, theInst);
    }
}

/*! \brief Volume handling on AVRCP disconnect
    \param theInst The disconnecting instance.
*/
void appAvVolumeHandleAvrcpDisconnect(avInstanceTaskData *theInst)
{
    avTaskData *theAv = appGetAv();
...
void appAvVolumeSet(uint8 volume, avInstanceTaskData *theOtherInst)
{
    ...
    for (int instance = 0; instance < AV_MAX_NUM_INSTANCES; instance++)
    {
        ...
        {
            if (appAvrcpIsConnected(theInst))
            {
                if (appDeviceIsHandset(bd_addr))
                {
                    /* Send new volume to source (Handset or TWS Master) */
                    DEBUG_LOGF("appAvVolumeSet, nofity volume %u to handset %p", volume, theInst);
            ...
            {
                DEBUG_LOGF("appAvVolumeSet, can't send volume %u, AVRCP not connected", volume, theInst);
            }
        }
    }
}
...
void appAvVolumeAttributeStore(avTaskData *theAv)
{
    ...
    {
        appSmGetPeerHandsetAddr(&bd_addr);
    }
