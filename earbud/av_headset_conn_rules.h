...
enum    av_headset_conn_rules_messages
{
    ...
    /*! Start timer to disconnect handset if earbud stays idle */
    CONN_RULES_IDLE_DISCONNECT_HANDSET,

    /*! Change page scan settings. */
    CONN_RULES_PAGE_SCAN_UPDATE,

    /*! Transfer SCO to Earbud */
    CONN_RULES_SCO_TRANSFER,

    /*! Make sure the peer link for SCO forwarding is established */
    CONN_RULES_SEND_PEER_SCOFWD_CONNECT,

    /*! Shutdown on low battery / no charger */
    CONN_RULES_LOW_POWER_SHUTDOWN,
...
#define RULE_EVENT_PEER_HFP_CONNETED             (1ULL << 35)    /*!< Peer handset HFP connected */
...
#define RULES_EVENT_BATTERY_TOO_LOW              (1ULL << 41)    /*!< Battery voltage is too low to proceed */
...
/*! \brief Determine if there are still rules in progress.
    \return bool TRUE there are still rules in progress.
                 FALSE there are no rules in progress.
    Note this function will ignore any rules that are marked with
    the flag #RULE_FLAG_ALWAYS_EVALUATE.
*/
