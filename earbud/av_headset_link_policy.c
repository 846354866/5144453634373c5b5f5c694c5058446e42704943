...
static void appLinkPolicyCheckRole(void)
{
    ...
    if (    appDeviceGetAttachedHandsetBdAddr(&handset)
        && !BdaddrIsZero(&handset)
        && !appDeviceIsTwsPlusHandset(&handset))
    {
        if (appScoFwdHasConnection() && theLp->scofwd_role != hci_role_master)
        {
            if (appScoFwdIsStreaming())
            {
                DEBUG_LOG("appLinkPolicyCheckRole. ScoFwd link is in the wrong state, and we can't change");
            }
            else 
            {
                DEBUG_LOG("appLinkPolicyCheckRole. Role switching ScoFwd link");
