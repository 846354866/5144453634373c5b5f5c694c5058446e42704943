...
#define AVRCP_PEER_CMD_STARTUP_SYNC_SIZE                 15         /*!< Message length */
...
#define AVRCP_PEER_CMD_STARTUP_SYNC_TX_SEQNUM_OFFSET     13
#define AVRCP_PEER_CMD_STARTUP_SYNC_RX_SEQNUM_OFFSET     14
...
#define AVRCP_PEER_CMD_STARTUP_SYNC_RULES_IN_PROGRESS           (1 << 2)
/*!@} */
    ...
    if (ind->size_payload == AVRCP_PEER_CMD_STARTUP_SYNC_SIZE)
    {
            ...
            AVRCP_PEER_CMD_ADD_LINK_KEY_ADDR_TYPE_BREDR)
        {
            if (peer_sig->rx_sync_task)
            {
                ...
                message->peer_rules_in_progress   = (pairing & AVRCP_PEER_CMD_STARTUP_SYNC_RULES_IN_PROGRESS) ? 1 : 0;
                message->tx_seqnum                = ind->payload[AVRCP_PEER_CMD_STARTUP_SYNC_TX_SEQNUM_OFFSET];
                message->rx_seqnum                = ind->payload[AVRCP_PEER_CMD_STARTUP_SYNC_RX_SEQNUM_OFFSET];
                ...
                DEBUG_LOGF("appPeerSigHandleSyncCommand, peer_is_pairing %u, peer_has_handset_pairing %u txseq %u rxseq %u RIP %u",
                           message->peer_is_pairing, message->peer_has_handset_pairing, message->tx_seqnum, message->rx_seqnum, message->peer_rules_in_progress);

                MessageSend(peer_sig->rx_sync_task, PEER_SIG_SYNC_IND, message);
                return TRUE;
            }
        }
    }
...
static void appPeerSigHandleInternalSyncRequest(PEER_SIG_INTERNAL_SYNC_REQ_T* req)
{
    ...
    switch (appPeerSigGetState())
    {
        case PEER_SIG_STATE_CONNECTED:
        {
            uint8 message[AVRCP_PEER_CMD_PAIR_HANDSET_ADDRESS_SIZE];
            int index;
            const uint8 state = (req->sync_data.a2dp_connected  ? AVRCP_PEER_CMD_STARTUP_SYNC_STATE_A2DP_CONNECTED  : 0) +
                                (req->sync_data.a2dp_streaming  ? AVRCP_PEER_CMD_STARTUP_SYNC_STATE_A2DP_STREAMING  : 0) +
                                (req->sync_data.avrcp_connected ? AVRCP_PEER_CMD_STARTUP_SYNC_STATE_AVRCP_CONNECTED : 0) +
                                (req->sync_data.hfp_connected   ? AVRCP_PEER_CMD_STARTUP_SYNC_STATE_HFP_CONNECTED   : 0) +
                                (req->sync_data.is_startup      ? AVRCP_PEER_CMD_STARTUP_SYNC_STATE_STARTUP : 0) +
                                (req->sync_data.in_case         ? AVRCP_PEER_CMD_STARTUP_SYNC_STATE_IN_CASE : 0) +
                                (req->sync_data.in_ear          ? AVRCP_PEER_CMD_STARTUP_SYNC_STATE_IN_EAR : 0);

            const uint8 pairing = (req->sync_data.is_pairing           ? AVRCP_PEER_CMD_STARTUP_SYNC_PAIRING_HANDSET_IN_PROGRESS : 0) +
                                  (req->sync_data.have_handset_pairing ? AVRCP_PEER_CMD_STARTUP_SYNC_PAIRING_HANDSET_COMPLETE : 0) +
                                  (req->sync_data.peer_rules_in_progress ? AVRCP_PEER_CMD_STARTUP_SYNC_RULES_IN_PROGRESS : 0);
            ...
            DEBUG_LOGF("appPeerSigHandleInternalSyncReq, battery %u, bdaddr %04x,%02x,%06lx, version %u.%02u, state %x, startup %u, in_case %u, in_ear %u",
                       req->sync_data.battery_level, req->sync_data.handset_addr.nap, req->sync_data.handset_addr.uap, req->sync_data.handset_addr.lap,
                       req->sync_data.handset_tws_version >> 8, req->sync_data.handset_tws_version & 0xFF, state, req->sync_data.is_startup,
                       req->sync_data.in_case, req->sync_data.in_ear);
            DEBUG_LOGF("appPeerSigHandleInternalSyncReq, is_pairing %u, have_handset_pairing %u RIP %u",
                       req->sync_data.is_pairing, req->sync_data.have_handset_pairing, req->sync_data.peer_rules_in_progress);

            index = AVRCP_PEER_CMD_STARTUP_SYNC_BATT_OFFSET;
            message[index++] = req->sync_data.battery_level & 0xFF;
            message[index] = (req->sync_data.battery_level >> 8) & 0xFF;

            index = AVRCP_PEER_CMD_STARTUP_SYNC_ADDR_TYPE_OFFSET;
            message[index] = AVRCP_PEER_CMD_ADD_LINK_KEY_ADDR_TYPE_BREDR;

            index = AVRCP_PEER_CMD_STARTUP_SYNC_ADDR_OFFSET;
            message[index++] =  req->sync_data.handset_addr.lap & 0xFF;
            message[index++] = (req->sync_data.handset_addr.lap >> 8) & 0xFF;
            message[index++] = (req->sync_data.handset_addr.lap >> 16) & 0xFF;
            message[index++] =  req->sync_data.handset_addr.uap;
            message[index++] =  req->sync_data.handset_addr.nap & 0xFF;
            message[index] = (req->sync_data.handset_addr.nap >> 8) & 0xFF;

            index = AVRCP_PEER_CMD_STARTUP_SYNC_TWS_VERSION_OFFSET;
            message[index++] = req->sync_data.handset_tws_version & 0xFF;
            message[index] = (req->sync_data.handset_tws_version >> 8) & 0xFF;
            ...
            message[AVRCP_PEER_CMD_STARTUP_SYNC_TX_SEQNUM_OFFSET] = req->sync_data.tx_seqnum;
            message[AVRCP_PEER_CMD_STARTUP_SYNC_RX_SEQNUM_OFFSET] = req->sync_data.rx_seqnum;
...
void appPeerSigSyncRequest(Task task, const bdaddr *peer_addr, peerSigSyncReqData* sync_data)
{
    ...
    DEBUG_LOGF("appPeerSigSyncRequest, battery %u, bdaddr %04x,%02x,%06lx, version %u.%02u",
               sync_data->battery_level,
               sync_data->handset_addr.nap, sync_data->handset_addr.uap, sync_data->handset_addr.lap,
               sync_data->handset_tws_version >> 8, sync_data->handset_tws_version & 0xFF);
    DEBUG_LOGF("appPeerSigSyncRequest, a2dp_connected %u, avrcp_connected %u, hfp_connected %u, a2dp_streaming %u",
                sync_data->a2dp_connected, sync_data->avrcp_connected,
                sync_data->hfp_connected, sync_data->a2dp_streaming);
    DEBUG_LOGF("appPeerSigSyncRequest, is_startup %u, in_case %u, in_ear %u, is_pairing %u, have_handset_pairing %u",
                sync_data->is_startup, sync_data->in_case, sync_data->in_ear,
                sync_data->is_pairing, sync_data->have_handset_pairing);
    DEBUG_LOGF("appPeerSigSyncRequest, txseq %u rxseq %u RIP %u", sync_data->tx_seqnum, sync_data->rx_seqnum, sync_data->peer_rules_in_progress);

    /* Populate message */
    message->client_task = task;
    message->sync_data = *sync_data;
...
{
    peerSigTaskData* peer_sig = appGetPeerSig();
    peer_sig->rx_sync_task = startup_sync_task;
}
